...
import { MaterialColumnForm } from '@/components/materials/material-column-form';
import {
    useMaterialColumns,
    useCreateMaterialColumn,
    useUpdateMaterialColumn,
    useDeleteMaterialColumn,
} from '@/hooks/use-material-columns';
import { ColumnType, type MaterialColumn, type CreateMaterialColumnDto } from '@/types';
...
interface SortableColumnItemProps {
    ...
    onEdit: (column: MaterialColumn) => void;
    onDelete: (column: MaterialColumn) => void;
}
...
interface MaterialColumnManagerProps {
    jobId: string;
}

export function MaterialColumnManager({ jobId }: MaterialColumnManagerProps) {
    const { data: columns, isLoading, error } = useMaterialColumns(jobId);
    const createColumn = useCreateMaterialColumn();
    const updateColumn = useUpdateMaterialColumn();
    const deleteColumn = useDeleteMaterialColumn();

    const [isAdding, setIsAdding] = useState(false);
    const [editingColumn, setEditingColumn] = useState<MaterialColumn | null>(null);
    const [deletingColumn, setDeletingColumn] = useState<MaterialColumn | null>(null);
    ...
    const handleCreate = async (data: CreateMaterialColumnDto) => {
        try {
    ...
    const handleUpdate = async (data: CreateMaterialColumnDto) => {
    ...
    if (error) {
        return (
            <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-destructive">
                Failed to load material columns. Please try again.
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">Material Columns</h3>
            ...
            {isLoading ? (
                ...
                <div className="flex h-24 items-center justify-center rounded-lg border border-dashed text-muted-foreground">
                    No material columns defined. Add columns to structure your material data.
                </div>
            )}
            ...
            <Dialog open={isAdding} onOpenChange={setIsAdding}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Add Material Column</DialogTitle>
                        <DialogDescription>
                            Define a new column for the material list
                        </DialogDescription>
                    </DialogHeader>
                    <MaterialColumnForm
                        nextOrder={nextOrder}
            ...
            <Dialog open={!!editingColumn} onOpenChange={() => setEditingColumn(null)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Edit Material Column</DialogTitle>
            ...
            <AlertDialog open={!!deletingColumn} onOpenChange={() => setDeletingColumn(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Delete Material Column</AlertDialogTitle>
                        <AlertDialogDescription>
                            Are you sure you want to delete the &quot;{deletingColumn?.name}&quot; column?
                            This will remove this field from all existing materials.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
